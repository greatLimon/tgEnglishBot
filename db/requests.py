    ...
    async with async_session() as session:
        ...
        if found_word:
            ...
            if not found_word_id in (1,2,3,4):
                ...
                if users_words:
                    only_user = True
                    if len(users_words.all()) > 1:
                        for word in users_words.all():
...
async def get_userwords(user_id:str)->list | None:
    async with async_session() as session:
            result_list = []
            userwords = await session.scalars(select(UsersWords).where(UsersWords.user_id == user_id))
            for word in userwords.all():
                q_words = await session.scalar(select(Words).where(Words.id == word.word_id))
                result_list.append((q_words.word_ru, q_words.word_en))
            return result_list
